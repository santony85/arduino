...
int DFRobot_SIM808::sendData(const char * str, const char * retAtt)
{
    delay(500);
    sim808_send_cmd(str);
    delay(500);
    sim808_send_End_Mark();
    if(!sim808_wait_for_resp(retAtt, DATA, DEFAULT_TIMEOUT * 10, DEFAULT_INTERCHAR_TIMEOUT * 10)) {
        return 0;
    }      
...
bool DFRobot_SIM808::getGPS() 
{
   ...
   // tok = strtok(NULL, ",");
    ...
    if(latdir[0]=='S')GPSdata.lat = 0 -GPSdata.lat;
    // convert longitude from minutes to decimal
	GPSdata.lon = (int)(longitude / 100) + (longitude - (int)(longitude / 100) * 100) / 60;
    if(longdir[0]=='W')GPSdata.lon = 0 -GPSdata.lon;
