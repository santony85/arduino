...
MqttError MqttBroker::publish(const MqttClient *source, const Topic &topic, MqttMessage &msg) const
{
	...
	for (auto client : clients)
	{
		...
		Serial << "brk_" << (broker && broker->connected() ? "con" : "dis") << "	srce=" << (source->isLocal() ? "loc" : "rem") << " clt#" << i << ", local=" << client->isLocal() << ", con=" << client->connected() << endl;
#endif
		bool doit = false;
...
void MqttClient::processMessage(MqttMessage *mesg)
{
	...
	{
#ifdef NOT_ESP_CORE
		Serial << "---> INCOMING " << _HEX(mesg->type()) << " client(" << (dbg_ptr)client << ':' << clientId << ") mem="
			   << " ESP.getFreeHeap() " << endl;
#else
...
MqttError MqttClient::publish(const Topic &topic, const char *payload, size_t pay_length)
{
	...
	Serial << "--> data publish " << topic.c_str() << String(len) << endl;
...
void MqttMessage::encodeLength()
{
	if (state != Complete)
	{
		...
		buffer[2] = (length >> 7);
		vheader = 3;
		Serial << "Taille " << String(length) << " bytes" << endl;
...
MqttError MqttMessage::sendTo(MqttClient *client)
{
	if (buffer.size())
	{
		Serial << "sending " << buffer.size() << " bytes" << endl;
		encodeLength();
		//hexdump("snd");
...
void MqttMessage::hexdump(const char *prefix) const
{
	...
	for (const char chr : buffer)
	{
		...
		{
			if (ascii.length())
				Serial << hex_to_str << ascii << separator << endl;
			if (prefix)
