...
class MqttClient
{
	using CallBack = int (*)(const MqttClient *source, const Topic &topic, const char *payload, size_t payload_length);
...
public:
	...
	bool connected() { return (parent != nullptr and client == nullptr) or
	...
	void setCallback(CallBack fun)
	{
		...
		if (callback)
			callback(this, "test/topic", "value", 5);
#endif
	};
