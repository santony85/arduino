...
bool CJY901::readSerialData(uint8_t data)
{
	...
	case 0x55:
		...
		break; //端口状态
	case 0x56:
		memcpy(&JY901_data.pressure, &rxBuffer[2], 4); //气压
		memcpy(&JY901_data.altitude, &rxBuffer[6], 4); //高度
		break;
	case 0x57:
		memcpy(&JY901_data.lon, &rxBuffer[2], 4); //经度
		memcpy(&JY901_data.lat, &rxBuffer[6], 4); //纬度
		break;
	case 0x58:
		memcpy(&JY901_data.GPSHeight, &rxBuffer[2], 2); //地速
		memcpy(&JY901_data.GPSYaw, &rxBuffer[4], 2);
		memcpy(&JY901_data.GPSVelocity, &rxBuffer[6], 4);
		break;
	case 0x59:
		memcpy(&JY901_data.quaternion, &rxBuffer[2], 10);
		break; //磁场
	}
	lastTime = millis();
	return true;
...
{
	if (transferMode_)
		readRegisters(address_, JY_Q0, 2, (uint8_t *)&JY901_data.quaternion.q0);
	return JY901_data.quaternion.q0 / (32768.0);
}

double CJY901::getQ1()
{
	if (transferMode_)
		readRegisters(address_, JY_Q1, 2, (uint8_t *)&JY901_data.quaternion.q1);
	return JY901_data.quaternion.q1 / (32768.0);
}

double CJY901::getQ2()
{
	if (transferMode_)
		readRegisters(address_, JY_Q2, 2, (uint8_t *)&JY901_data.quaternion.q2);
	return JY901_data.quaternion.q2 / (32768.0);
}

double CJY901::getQ3()
{
	if (transferMode_)
		readRegisters(address_, JY_Q3, 2, (uint8_t *)&JY901_data.quaternion.q3);
	return JY901_data.quaternion.q3 / (32768.0);
}
